...
coord = [[0,0], [3, .3]]
global_coord = [[1.6, -1.4] , [1.6, -.13], [1.6, 1.4]]
...
class RobotController:
    ...
    def __init__(self, coord, global_coord):
        ...
        self.sub_stop2 = rospy.Subscriber('int_stop2', Float32, self.sub_stop_callback)
        self.sub_emergency_stop = rospy.Subscriber('emergency_stop2', Bool, self.emergency_stop_callback)

        self.pub = rospy.Publisher('robot_1_vel', Twist, queue_size=10)
        self.pub_stop1 = rospy.Publisher('int_stop1', Float32, queue_size=10)
        self.pub_e_stop = rospy.Publisher('emergency_stop1', Bool, queue_size=10)
    ...
    def pid_test(self, start_pos, end_pos):
        ...
        while not rospy.is_shutdown(): #and not self.stop:
            ...
            if math.hypot(global_coord[1][0] - amcl_pos[0], global_coord[1][1] - amcl_pos[1]) < goal_threshold:
                #Publish timer to /int_stop1 topic
                elapsed_time = self.time_since_stopped()
                int_msg = Float32()
                int_msg.data = float(elapsed_time)
                self.pub_stop1.publish(int_msg)
                #Wait 5 seconds at intersection if arrives first
                if elapsed_time == 0.0 or elapsed_time < 5:
                    self.stop_robot()
                    continue
                #If robot2 has waited longer than robot1, stop
                elif elapsed_time > 0.0 and self.stop_time2 > self.stop_time1:
                    self.stop_robot()
                #Checks if robot2 is crossing intersection
                elif self.stop_time2 > 5 and self.stop:
                    self.stop_robot()
    ...
    def stop_robot(self):
        ...
        if self.stop_time1 is 0:
            self.stop_time1 = time.time()
    ...
    def time_since_stopped(self):
        ...
        '''
        if self.stop_time1 != 0:
            count = time.time() - self.stop_time1
            print(count)
            return count
        else:
            return 0.0
